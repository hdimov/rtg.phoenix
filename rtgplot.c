...
//  long datapoints;		// Number of datapoints in range
	...
	if (arguments.cs > 0) {
		...
		while (clr > 0) { clr--; colors = colors -> next; }
	...
	if (arguments.diff_mode == 4) { // NEW STYLE; use RTG-PARSER data structure;
		...
		for (i = 0; i < groups_count; i++) {
			dpc = 0; max_dpc = 0;// zero fukcing thing;
			...
			for(j = 0; j < groups[i].tables_count; j++) {
				...
				for(k = 0; k < groups[i].tables[j].ids_count; k++) {
					...
					// fprintf(dfp, "  query: %s\n", query);
					if ( (dpc = populate(query, &mysql, &groups[i].tables[j].data[k], &graph)) < 0) { // populate sets max x value;
					
						if (set.verbose >= HIGH) 
							fprintf(dfp, 
									"NO DATA for table: %s and id: %d!. continue...\n", groups[i].tables[j].name, groups[i].tables[j].ids[k]);
						continue;
						...
						if (graph.range.end - graph.range.dataBegin > graph.xmax) graph.xmax = graph.range.end - graph.range.dataBegin;
						...
						if (groups[i].tables[j].rate[k].max > graph.ymax) graph.ymax = groups[i].tables[j].rate[k].max; // this one is responcible for max y value;
					...
					for (i = 0; i < groups_count; i++) {
						...
						for (j = 1; j < groups[i].aggr_count; j++) {
							...
							if (j < groups[i].aggr_count - 1) {
								...
								if (groups[i].tables[groups[i].aggr_table_idx[j]].rate[groups[i].aggr_rate_idx[j]].cur > 2048.00)
							...
							plot_legend2(&img, groups[i].tables[groups[i].aggr_table_idx[j]].rate[groups[i].aggr_rate_idx[j]], &graph, colors->shade, counter_name, &offset);
							colors = colors -> next;
							offset++;
...
//					fprintf(dfp, "aggregate == 2: begin\n");
					for (i = 0; i < groups_count; i++) {
						...
						// plot_legend(&img, groups[i].aggr_rate[j], &graph, colors->shade, groups[i].name, offset);
						plot_legend2(&img, groups[i].aggr_rate[groups[i].aggr_count - 1], &graph, colors->shade, groups[i].name, &offset);
						colors = colors -> next;
						offset++;
		...
		for (i = 0; i < arguments.tables_to_plot; i++) {
			...
			for (j = 0; j < arguments.iids_to_plot; j++) {
				...
				if (graph.impulses || graph.gauge) { // use this only for gauges;
					...
					/* maximum Y value is largest of all line rates */
					if (!graph.scaley && (rate[i][j].max > graph.ymax)) graph.ymax = rate[i][j].max; // this one is responcible for max y value;
				}
			}
		}
				...
				if (arguments.aggregate) {
					...
					for (j = 1; j < arguments.iids_to_plot; j++) {
						if (data[i][j] && data[i][j]->next) {
							...
							normalize(data[i][j], &graph); // go and make everything aligned by graph -> maxx and graph -> maxy;
						} else {
...
int populate(char *query, MYSQL * mysql, data_t ** data, graph_t * graph) {
	...
	/* no data, go home */
	if (*data == NULL) {
		if (set.verbose >= DEBUG) fprintf(dfp, "  No Data Points %ld in %ld Seconds.\n", range->datapoints, range->end - range->begin);
		return (-1);
	} else {
		...
		if (set.verbose >= DEBUG) fprintf(dfp, "  %ld Data Points in %ld Seconds.\n", range->datapoints, range->end - range->dataBegin);
	}
...
void plot_scale(gdImagePtr *img, graph_t *graph) {
	...
	for (i = 0; i <= graph->image.yplot_area; i += (graph->image.yplot_area / YTICKS)) {
		...
		if (graph->ymax / graph->yunits < 10) {
			...
			gdImageString(*img, gdFontSmall, BORDER_L - 30, BORDER_T + graph->image.yplot_area - i - 5, string, std_colors[black]);
			continue;
...
void plot_labels(gdImagePtr * img, graph_t * graph, arguments_t* args) {
	...
	gdImageStringUp(*img, gdFontSmall, BORDER_L - 45, BORDER_T + (graph->image.yplot_area * 2 / 3), string, std_colors[black]);
...
/*	int		red[MAXLINES] =  {  0,   0, 255,   0, 255, 255, 255,   0, 132,   0, 132, 132, 132, 102,  0, 64};
	...
	int		red[MAXLINES] =  {  0,   0, 255,   0, 255, 255, 255,  51, 255,  51,   0, 173,  51, 184, 255, 102, 255, 204,   0, 245, 112, 56};
	int		green[MAXLINES] ={255,   0,   0, 255,   0, 255, 153, 204, 102, 102, 138, 153, 255,  46, 204,  51,  51,  51, 153,  61, 219, 128};
	int		blue[MAXLINES] = {  0, 255,   0, 255, 255,   0,   0, 255,  51, 255, 184, 255, 204,   0,  51, 255, 204, 255, 255,   0, 255, 113};
...
int getCounterDescription(MYSQL *mysql, char *res, char *dir, int id) {
	...
	bzero(query, 256);
	snprintf(query, sizeof(query), "select t1.name, t2.cust_sh_name, t1.if_name, t2.if_alias, substring_index(t2.host, '.', 2) as host from interface as t1, service_map as t2 where t1.id = %d and t1.rid = t2.tbl_rid and t1.if_name = t2.if_name;", id);
