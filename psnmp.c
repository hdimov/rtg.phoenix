...
int _buffered_print_result(int thread_id, struct session *_host_ss, int status, struct snmp_session *sp, struct snmp_pdu *response) {
...
void *sync_poller(void *thread_args) {
	...
	for( ; ; ) {
		...
		_buffered_print_result(worker -> index, _host_ss, _status, &_sess, response);
